...
<script src="{{  asset('js/toastr.js')  }}"></script>
    <div class="row">
        <div class="col-sm-6">
            ...
            @endif
            <form action="{{ url('/post/update/id='.$post->id) }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                            ...
                            <select required class="select2 form-control" name="adminId" id="sel1">
                                @foreach($admin as $admins)
                                    <option  value='{{$admins->id}}' @if($post->adminId == $admins->id) selected="selected" @endif>{{$admins->name}}</option>
                                @endforeach
                            </select>
                    </div>
                    <div class="form-group">
                            ...
                            <select required class="select2 form-control" name="categoryId" id="sel1">
                                @foreach($genre as $genres)
                                    <option value='{{$genres->id}}' @if($post->categoryId == $genres->id) selected="selected" @endif>{{$genres->name}}</option>
                                @endforeach
                            </select>
                    </div>
                    <div class="form-group">
